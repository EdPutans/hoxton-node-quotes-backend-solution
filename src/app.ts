...
import storedQuotes from "./data/quotes";
import { Quote, Request } from "./types";
...
app.post("/quotes", (req: Request, res) => {
  ...
  if (missingProperty)
    return res
      .status(400)
      .send(`${missingProperty} is missing! Check the request body!`);
...
app.patch("/quotes/:id", (req: Request, res) => {
  ...
  if (index === -1)
    return res.status(404).send("Nobody probably said that yet. 404.");
...
app.delete("/quotes/:id", (req: Request, res) => {
  ...
  res.status(200).send('removed quote');
});
